...
let lambda: LambdaClient, s3: S3Client, supabase: any, pusher: Pusher, openai: OpenAI
...
try {
  ...
  pusher = new Pusher({ appId: appId!, key: pubKey!, secret: secret!, cluster: "eu", useTLS: true })
 ...
 */
const validateInput = (payload: any): { valid: boolean; error?: string } => {
 ...
 */
const generateRegionalChunks = async (location: string): Promise<RegionChunk[]> => {
  try {
    ...
    const chunks = extractJsonFromResponse(content) as RegionChunk[]
    
    const isValidChunk = (chunk: any): chunk is RegionChunk => 
      chunk && typeof chunk.region === 'string' && typeof chunk.location === 'string'
 ...
 */
const getFallbackChunks = (location: string): RegionChunk[] => {
  ...
  results.forEach((result, index) => {
    if (result.status === "fulfilled") {
      result.value.forEach(place => {
        if (place.place_id && !allPlaces.has(place.place_id)) allPlaces.set(place.place_id, place)
      })
    } else {
      console.error(`Query ${index + 1} failed:`, (result as PromiseRejectedResult).reason)
    }
  })
  ...
  for (let i = 0; i < places.length && leads.length < limit; i += batchSize) {
    ...
    const batchPromises = batch.map(async (place, batchIndex) => {
      try {
        ...
        const res = await fetch(detailsUrl, { timeout: 8000 })
        if (!res.ok) {
        ...
        if (normalizedCompany && seen.companies.has(normalizedCompany)) return null
        if (normalizedPhone && normalizedPhone.length > 5 && seen.phones.has(normalizedPhone)) return null
        
        const email = website ? await extractEmailSafely(website) : ""
        if (email && seen.emails.has(email.toLowerCase())) return null
        
        if (normalizedCompany) seen.companies.add(normalizedCompany)
        if (normalizedPhone) seen.phones.add(normalizedPhone)
    ...
    const validLeads = batchResults
      .filter((r): r is PromiseFulfilledResult<Lead> => r.status === 'fulfilled' && r.value !== null)
      .map(r => r.value)
...
const extractEmailSafely = async (url: string): Promise<string> => {
  try {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => 
      setTimeout(() => reject(new Error('Email extraction timeout')), 4000)
    )
    ...
    const validEmail = emails?.find(e => 
      !/(example|test|placeholder|noreply|no-reply|admin|info@example)/.test(e.toLowerCase()) &&
      e.length < 50
    )
    ...
    if (!(error as Error).message.includes('timeout')) {
      console.warn(`Email extraction failed for ${url}:`, (error as Error).message)
    }
    return ""
  }
}
 ...
 */
const startProgressUpdater = (id: string, channelId: string, getCurrentCount: () => number, startTime: number) => {
 ...
 */
const invokeChildLambda = async (payload: JobPayload): Promise<{ success: boolean; region: string; error?: string }> => {
  try {
    ...
    return { success: false, region: payload.region || 'unknown', error: (error as Error).message }
  }
}
 ...
 */
const checkAndMergeResults = async (parentId: string, channelId: string): Promise<void> => {
  try {
    ...
    await pusher.trigger(channelId, "scraper:error", { id: parentId, error: `Merge failed: ${(error as Error).message}` })
  }
}
...
function formatDuration(seconds: number): string {
  if (seconds < 0) return "0s" // Handle negative input
  if (seconds < 60) return `${Math.floor(seconds)}s` // Less than 1 minute
  ...
  try {
    ...
    if (!isChildJob && limit > MAX_LEADS_PER_JOB) {
      ...
      const invocationResults = await Promise.allSettled(
        childJobs.map((job: Job) => invokeChildLambda({
          keyword,
      ...
      const successful = invocationResults.filter(r => r.status === 'fulfilled' && (r.value as any).success).length;
      const failed = invocationResults.filter(r => r.status === 'rejected' || (r.status === 'fulfilled' && !(r.value as any).success));
      ...
      if (successful === 0) throw new Error("All child Lambda invocations failed");
    ...
    try {
      const leads = await scrapePlaces(keyword, location, limit, (count: number) => {
      ...
      if (isChildJob && parentId) {
        ...
        if (fetchError) throw new Error(`Failed to fetch child jobs: ${fetchError.message}`);

        const completedCount = childJobs.filter((job:Job) => job.status === "completed").length;
        const totalLeads = childJobs.reduce((sum: any, job: Job) => sum + job.leads_count, 0);
      ...
      const processingTime = Math.round((Date.now() - start) / 1000);
      const baseErrorMessage = `Scraping failed: ${(scrapeError as Error).message || String(scrapeError)}`;
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          type: (scrapeError as Error).constructor?.name || 'Unknown'
        })
      };
    }
  } catch (error: unknown) {
    const processingTime = Math.round((Date.now() - start) / 1000);
    const baseErrorMessage = `Critical error: ${(error as Error).message || String(error)}`;
    ...
    console.error("   Type:", (error as Error).constructor?.name || 'Unknown');
    console.error("   Stack:", (error as Error).stack?.slice(0, 1000));
