...
const handler = async (event) => {
    ...
    try {
        ...
        if (IS_DEBUGGING)
            executionLogs += `[debug] Lambda execution started with payload: ${JSON.stringify(event, null, 2)}\n`;
        ...
        if (IS_DEBUGGING)
            executionLogs += `[debug] Available SDKs: ${availableSDKs.join(', ')}\n`;
        ...
        if (!citiesToScrape.length) {
            ...
            if (IS_DEBUGGING)
                executionLogs += `[debug] Full cities list: ${citiesToScrape.join(', ')}\n`;
        }
        ...
        const shouldRetry = foundRatio < 0.8 && retryCount < exports.MAX_RETRIES && remainingLeads > 0 && remainingLeads <= MAX_LEADS_PER_JOB && newLeadsFound > 0;
        if (shouldRetry) {
