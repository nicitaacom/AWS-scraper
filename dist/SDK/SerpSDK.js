...
class SerpSDK {
    ...
    async searchBusinesses(query, location, limit = 10) {
        ...
        try {
            ...
            // 3️⃣ Map to Lead[]
            const leads = await Promise.all(results.slice(0, limit).map(async (item) => {
                try {
            ...
            const valid = leads.filter((l) => l !== null);
    ...
    async callSerp(params) {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 15000);
        const url = this.endpoint + "?" + new URLSearchParams({ ...params, api_key: this.apiKey }).toString();
    ...
    async scrapePhoneFromWebsite(website) {
        ...
        const match = clean.match(/\b\+?[0-9][0-9\-\s().]{7,}\b/)?.[0] || "";
    ...
    extractAddress(snippet, location) {
        const street = snippet.match(/\d+\s+[\w\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Lane|Ln)\b/)?.[0];
