...
export class SerpSDK {
  ...
  public async searchBusinesses(query: string, location: string, limit = 10): Promise<Lead[] | string> {
    ...
    if (limit > 100) return "Limit exceeds free tier maximum of 100"
    ...
    try {
      ...
      if (data.error) return `SerpAPI error: ${data.error}`
      ...
      const leads = await Promise.all(
        allResults.slice(0, limit).map(async (item: any): Promise<Lead | null> => {
          try {
            ...
            const [email, phone] = await Promise.all([
              website ? scrapeEmailFromWebsite(website).catch(() => "") : "",
              item.phone ? 
      ...
      const validLeads = leads.filter((lead): lead is Lead => 
        lead !== null && Boolean(lead.company?.trim())
      )
