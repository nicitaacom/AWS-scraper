...
export class PuppeteerGoogleMapsSDK {
  ...
  private async initProxyList(): Promise<void> {
    try {
      const res = await fetch("https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks5&timeout=1000&country=all&ssl=all&anonymity=all", {
        timeout: 10000
      })
   ...
   */
  public async searchBusinesses(query: string, location: string, limit = 20): Promise<Lead[] | string> {
    ...
    try {
      ...
      if (!validLeads.length) return "No businesses found with contact information for the given query."
  ...
  private async scrollToLoadResults(page: Page, limit: number): Promise<void> {
  ...
  private async extractLeads(page: Page, limit: number): Promise<Lead[]> {
    const raw = await page.evaluate((mx: number) => {
      const getText = (el: Element, selectors: string[], filter?: (text: string) => boolean): string => {
        for (const sel of selectors) {
          const nodes = Array.from(el.querySelectorAll(sel))
          for (const node of nodes) {
            const txt = node.textContent?.trim() ?? ""
            if (txt && (!filter || filter(txt))) return txt
          }
        }
        return ""
      }
      ...
      return items.slice(0, mx).map((el, index) => {
        try {
          ...
          const websiteEl = el.querySelector('a[href*="http"]:not([href*="google.com"]):not([href*="maps"])') as HTMLAnchorElement
          ...
          } as Lead & { category?: string; extractedAt: string }
        } catch (error) {
    ...
    return raw.filter((lead): lead is Lead => lead !== null)
  }

  // 🎯 CRITICAL: Enrich leads with contact information
  private async enrichLeadsWithContactInfo(leads: Lead[], browser: Browser): Promise<Lead[]> {
    ...
    for (let i = 0; i < leads.length; i++) {
      ...
      try {
        ...
        if (lead.website && !lead.email) {
          ...
          if (websiteContact.email) enrichedLead.email = websiteContact.email
          if (websiteContact.phone && !enrichedLead.phone) enrichedLead.phone = websiteContact.phone
        }
        ...
        if (!enrichedLead.email || !enrichedLead.phone) {
          ...
          if (googleContact.email && !enrichedLead.email) enrichedLead.email = googleContact.email
          if (googleContact.phone && !enrichedLead.phone) enrichedLead.phone = googleContact.phone
          if (googleContact.website && !enrichedLead.website) enrichedLead.website = googleContact.website
        }
        ...
        if (!enrichedLead.email || !enrichedLead.phone) {
          ...
          if (directoryContact.email && !enrichedLead.email) enrichedLead.email = directoryContact.email
          if (directoryContact.phone && !enrichedLead.phone) enrichedLead.phone = directoryContact.phone
        }
  ...
  private async scrapeWebsiteForContact(websiteUrl: string, browser: Browser): Promise<{ email?: string; phone?: string }> {
    const page = await browser.newPage()
    try {
      await page.setUserAgent(this.userAgents[Math.floor(Math.random() * this.userAgents.length)])
  ...
  private async googleSearchForContact(company: string, address: string, browser: Browser): Promise<{ email?: string; phone?: string; website?: string }> {
    const page = await browser.newPage()
    try {
      await page.setUserAgent(this.userAgents[Math.floor(Math.random() * this.userAgents.length)])
  ...
  private async searchBusinessDirectories(company: string, address: string, browser: Browser): Promise<{ email?: string; phone?: string }> {
    const directories = [
