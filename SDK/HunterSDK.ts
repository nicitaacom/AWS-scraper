...
export class HunterSDK {
  ...
  private readonly rateLimitDelay = 6000 // 6 seconds (10 req/min for free tier)
  ...
  private async enrichLead(domain: DomainInfo, location: string): Promise<Lead> {
    try {
      ...
      if (domain.website) {
        try {
          const contacts = await scrapeContactsFromWebsite(domain.website)
  ...
  private extractDomainsFromHtml(html: string, limit: number): DomainInfo[] {
    ...
    for (const pattern of patterns) {
      ...
      while ((match = pattern.exec(html)) && domains.length < limit) {
        ...
        if (!this.isValidDomain(cleanDomain) || seenDomains.has(cleanDomain)) continue
  ...
  private async findEmails(domain: string): Promise<string[] | string> {
    try {
      ...
      return emails.filter((email: string) => email && email.includes('@'))
    } catch (error: unknown) {
